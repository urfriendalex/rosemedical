...
export function BrandPage({ brand, data, locale }: { brand: Brand; data: SiteData; locale: Locale }) {
  const currentIndex = data.brands.findIndex((item) => item.slug === brand.slug);
  const nextBrand = data.brands[(currentIndex + 1) % data.brands.length] ?? data.brands[0];
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="relative flex min-h-screen flex-col justify-end px-5 pb-10 pt-28 sm:px-8 lg:px-12 lg:pb-16">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-4xl">
              <Link href={`${localePath[locale]}/#brands`} className="text-sm font-medium text-white/72 transition hover:text-white">
            ...
            <div className="flex shrink-0 flex-col gap-3 sm:flex-row lg:pb-1">
              <a className="hero-primary-button" href={brand.catalog[locale]}>
                ...
                <span className="hero-primary-icon masked-arrow axis-y" aria-hidden>
                  <Download className="arrow-one h-4 w-4" />
                  <Download className="arrow-two h-4 w-4" />
                </span>
              </a>
              <AnimatedLink variant="secondary" href={`${localePath[locale]}#contact`}>
                {locale === "en" ? "Ask about availability" : "Zapytaj o dostepnosc"}
              </AnimatedLink>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-[1664px] gap-8 px-5 py-24 sm:px-8 lg:grid-cols-2 lg:px-12">
        <InfoPanel title={locale === "en" ? "Product categories" : "Kategorie produktow"} items={brand.categories.map((item) => localized(item, locale))} />
        <InfoPanel title={locale === "en" ? "Notes" : "Informacje"} items={brand.notes.map((item) => localized(item, locale))} />
      </section>
        ...
        <div className="mt-20 grid gap-10 lg:grid-cols-[324px_1fr] lg:gap-[11px]">
          ...
          <div className="grid gap-[11px] md:grid-cols-3">
            {home.features.map((feature, index) => (
              <article key={localized(feature.title, locale)} className="surface-card group p-8">
                <ShieldCheck className="mb-10 text-accent transition duration-300 group-hover:translate-y-[-3px]" size={24} aria-hidden />
                <AnimatedHeading as="h3" delay={index * 0.08} className="text-[28px] font-medium leading-[1.05] tracking-[-0.035em]">
                  {localized(feature.title, locale)}
                </AnimatedHeading>
                <AnimatedText delay={0.18 + index * 0.08} className="mt-5 text-sm leading-[1.5] text-muted-foreground">
...
function BenefitsSection({ data, locale }: { data: SiteData; locale: Locale }) {
  ...
  return (
    <section id="products" className="bg-surface px-5 py-28 sm:px-8 lg:px-12 lg:py-32">
      <div className="mx-auto max-w-[1664px]">
        ...
        <div className="benefit-grid relative grid grid-cols-1 md:grid-cols-3">
          ...
          {home.features.map((feature, index) => (
            <article key={localized(feature.title, locale)} className="flex min-h-[620px] flex-col gap-8 p-8 lg:p-10">
              {index === 1 ? <BenefitImage src={images[index]} title={localized(feature.title, locale)} delay={index * 0.12} /> : null}
              <div className={index === 1 ? "mt-auto" : ""}>
                <div className="mb-4 flex items-start gap-3">
                  <span className="mt-2 text-xs text-muted-foreground">({String(index + 1).padStart(2, "0")})</span>
                  <AnimatedHeading as="h3" delay={index * 0.1} className="text-3xl font-medium leading-[1.05] tracking-[-0.035em]">
                    {localized(feature.title, locale)}
                  </AnimatedHeading>
                </div>
                <AnimatedText delay={0.2 + index * 0.1} className="max-w-sm text-sm leading-[1.5] text-muted-foreground">
                  {localized(feature.body, locale)}
                </AnimatedText>
              </div>
              {index !== 1 ? <div className="mt-auto"><BenefitImage src={images[index]} title={localized(feature.title, locale)} delay={index * 0.12} /></div> : null}
            </article>
          ))}
        </div>
      </div>
  ...
  return (
    <section id="contact" className="px-5 py-28 sm:px-8 lg:px-12 lg:py-32">
      <div className="mx-auto grid max-w-[1664px] gap-12 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          <p className="section-eyebrow">{localized(home.contact.eyebrow, locale)}</p>
          ...
          <div className="mt-10 grid gap-3 text-sm">
            <a className="inline-flex items-center gap-3 transition hover:text-accent" href={`mailto:${settings.contact.email}`}>
              <Mail aria-hidden size={17} /> {settings.contact.email}
            </a>
            <a className="inline-flex items-center gap-3 transition hover:text-accent" href={`tel:${settings.contact.phone.replaceAll(" ", "")}`}>
              <Phone aria-hidden size={17} /> {settings.contact.phone}
            </a>
          </div>
        </div>
        <div className="rounded-[28px] border border-border bg-white/70 p-5 shadow-[0_30px_90px_rgba(6,63,57,0.08)] backdrop-blur sm:p-8">
    ...
    <section className="px-5 pb-24 sm:px-8 lg:px-12">
      ...
      >
        <div className="p-8 sm:p-10 lg:p-12">
          ...
          <span className="mt-9 inline-flex min-h-12 items-center gap-3 rounded-full bg-white px-5 py-3 text-sm font-medium text-emerald-950">
            {locale === "en" ? "Browse next provider" : "Zobacz kolejnego dostawce"}
...
function BenefitImage({ src, title, delay }: { src: string; title: string; delay: number }) {
  return (
...
function InfoPanel({ title, items }: { title: string; items: string[] }) {
  return (
    <section className="rounded-[28px] border border-border bg-white/72 p-8">
      <h2 className="font-display text-3xl font-medium tracking-[-0.04em]">{title}</h2>
...
function SiteFooter({ data, locale }: { data: SiteData; locale: Locale }) {
  const addressParts = parseFooterAddress(localized(data.settings.contact.address, locale));
  ...
  return (
    <footer className="border-t border-border bg-background px-5 py-10 sm:px-8 lg:px-12">
      <div className="mx-auto grid max-w-[1664px] gap-8 lg:grid-cols-[1fr_auto]">
        <div>
          <div className="font-display text-3xl font-medium tracking-[-0.04em]">Rose Medical</div>
          <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">{localized(data.settings.footer, locale)}</p>
        </div>
        <div className="grid gap-2 text-sm text-muted-foreground lg:text-right">
          <a className="transition hover:text-foreground" href={`mailto:${data.settings.contact.email}`}>{data.settings.contact.email}</a>
          <a className="transition hover:text-foreground" href={`tel:${data.settings.contact.phone.replaceAll(" ", "")}`}>{data.settings.contact.phone}</a>
